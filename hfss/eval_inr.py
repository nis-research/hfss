...
import pickle
import sys
from utils import get_dataset,get_meanstd
...
imagenet_val_location = "/local/swang/ImageNet/val/"
...
def main(args):
    ...
    if not os.path.exists('IN_r/'):
        os.makedirs('IN_r/')
    sav_dir = 'IN_r/'
    save_result = sav_dir + args.model_name_abbr + '_' + stage + args.trial_index  +args.corruption+ '.csv'

    create_symlinks_to_imagenet("/local/swang/imagenet_val_for_imagenet_r/")
    ...
    classes = torch.range(0,999)
    classes = classes[imagenet_r_mask]
    # print(classes)
    for c_i in classes: 

        # print('class '+ str(c_i.item()))
        c_i = int(c_i.item())
...
#--------------
        transform=transforms.Compose([transforms.Resize(256),transforms.CenterCrop(args.img_size), transforms.ToTensor(),White_Mask(np.asarray(chosen_mask)),
        ...
        # print(len(data_test.targets))
        indices = torch.tensor(data_test.targets)
        class_id_inr = (classes == c_i).nonzero()

        # print(class_id_inr)
        # print(c_i)
        indices = (indices == class_id_inr.item()).nonzero() # if you want to keep images with the label 
        # print(indices)
        sampler = torch.utils.data.sampler.SubsetRandomSampler(indices)
        ...
        # print('Loaded data')
        num_correct = get_predictions(test_loader)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--corruption', type=str, default='IN_r',
    ...
    parser.add_argument('--dataset', type=str, default='imagenet_r',
                        help='dataset chosen')
